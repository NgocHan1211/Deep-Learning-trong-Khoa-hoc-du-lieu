...
from hf_resnet_model import PretrainedResnet 
...
os.makedirs("/kaggle/working/output_hf_resnet", exist_ok=True)
...
def evaluate(dataloader: DataLoader, model: nn.Module) -> dict:
    ...
    with torch.no_grad():
        for item in dataloader:
            ...
            output: torch.Tensor = model(image) # Chỗ này gọi hàm forward
...
if __name__ == "__main__":
    ...
    train_dataset = ImageFolderDataset(root_dir=TRAIN_DIR, transform=train_transform)
    test_dataset = ImageFolderDataset(root_dir=TEST_DIR, transform=test_transform)
    ...
    for epoch in range(EPOCH):
        ...
        if metrics[best_metric] > best_score:
            ...
            torch.save(model.state_dict(), "/kaggle/working/output_hf_resnet/best_hf_resnet50_model.pth")
    ...
    with open("/kaggle/working/output_hf_resnet/hf_resnet_history.json", "w") as f:
        json.dump(history_data, f)
        
    print("Đã lưu lịch sử vào '/kaggle/working/output_hf_resnet/hf_resnet_history.json'")
